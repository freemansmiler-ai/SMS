        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-slate-50 flex items-center gap-2">
              ...
              <span>Administrator Account Settings & Security</span>
            </h1>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              System governance profile details, authentication credentials, and session management.
            </p>
          </div>
        ...
        <div className="p-3 rounded-lg bg-emerald-50/70 dark:bg-emerald-950/30 border border-emerald-200/70 dark:border-emerald-800 text-xs flex items-center justify-between gap-2 text-emerald-900 dark:text-emerald-200">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">
              System Administrator Account: Authenticated with root management privileges over school operations.
            </span>
          </div>
        </div>
          ...
          <CardContent className="p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              <div className="h-16 w-16 rounded-full bg-slate-900 text-white font-black text-xl flex items-center justify-center border-2 border-slate-700 shrink-0">
                {profile?.name ? profile.name.split(" ").map((n) => n[0]).join("") : "ADM"}
              </div>
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <h2 className="text-lg font-bold text-slate-900 dark:text-slate-50">{profile?.name || "System Administrator"}</h2>
                ...
                <p className="text-xs text-slate-500 font-medium flex items-center gap-1">
                  ...
                  <span>{profile?.email || "administrator@academy.edu"}</span>
                </p>
              </div>
            </div>
          ...
          <Card className="border-slate-200/80 dark:border-slate-800">
            <CardHeader className="p-4 pb-3">
              <CardTitle className="text-sm font-bold flex items-center gap-2">
                ...
                <span>School Management Scoping</span>
              </CardTitle>
              <CardDescription className="text-xs">
                Administrative scope and authority limits.
              </CardDescription>
            </CardHeader>
            <CardContent className="p-4 pt-0 space-y-3 text-xs">
              <div className="p-3 rounded-md bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-700 space-y-1">
                <span className="text-[10px] font-bold text-slate-500 uppercase block">Governance Institution</span>
                <span className="font-bold text-slate-800 dark:text-slate-200 text-sm">Codivex Academy</span>
                <span className="font-mono text-slate-500 block text-[11px]">School Code: ABS-2026</span>
              </div>
              <div className="p-3 rounded-md bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-700 space-y-1">
                <span className="text-[10px] font-bold text-slate-500 uppercase block">Role Scope</span>
                <span className="font-bold text-slate-800 dark:text-slate-200">Full Administrator Privileges</span>
                <span className="text-slate-500 block text-[11px]">User Account Provisioning, Classes, Subjects & Staff Assignments</span>
              </div>
            </CardContent>
          </Card>
