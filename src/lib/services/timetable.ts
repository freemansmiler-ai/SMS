...
export async function fetchTimetableSlots(filters?: {
  ...
  try {
    ...
    const { data, error } = await (supabase.from("timetable_slots") as any)
      .select(`
        id,
        class_id,
        subject_id,
...
export async function createTimetableSlot(
  ...
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error } = await (supabase.from("timetable_slots") as any).insert({
...
export async function deleteTimetableSlot(id: string): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error } = await (supabase.from("timetable_slots") as any).delete().eq("id", id);
    if (error) return { success: false, error: error.message };
    return { success: true };
  } catch (err: unknown) {
